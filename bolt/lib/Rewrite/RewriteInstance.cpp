...
void RewriteInstance::patchBuildID() {
  ...
  LastIDByte ^= 1;
  if (opts::Rewrite) {
    OS.seek(FileOffset + IDOffset + BuildID.size() - 1);
    OS.write(&LastIDByte, 1);
  } else {
...
BinaryFunction *RewriteInstance::createPLTBinaryFunction(uint64_t TargetAddress,
  ...
  // its EntryAddress matches the section start. Register it as a pseudo
  // function so its bytes can be emitted, with a DATAat PLT symbol.
  if (!Symbol) {
...
void RewriteInstance::rewriteNoteSections() {
  ...
  if (opts::Rewrite) {
    ...
    NextAvailableOffset = getFileOffsetForAddress(NextAvailableAddress);
...
void RewriteInstance::writeEHFrameHeader() {
  ...
  if (opts::Rewrite) {
    ...
    if (NewEHFrameHdr.size() > EHFrameHdrSec->getOutputSize()) {
      errs() << "BOLT-WARNING: generated " << ".eh_frame_hdr"
                 << " (" << NewEHFrameHdr.size()

...
# RUN: A=`llvm-readelf -SW %t.bolt.kso | awk '$3==".got" {print $5}'` && P=`printf %x $(( 0x$A & ~0xfff ))` && O=`printf %x $(( 0x$A & 0xfff ))` && llvm-objdump -d --no-show-raw-insn %t.bolt.kso | FileCheck %s -DPAGE=$P -DOFF=$O
